...
//    Uint64toNumber(sd, (UInt64)m_inNames.size());
    ...
    // déterminer la taille de la liste des noms en UTF16 et séparés par des int16 à 0
    j = 3;                                         // j est le nombre d'octets entre début de kName et les noms eux même( kName, size, external
    temp64 = 0;
    for(i = 0; i < numFiles; i++)
    {
        int l;
    ...
    temp64++;                                       // taille de tous les noms + 1 (pour external)
    ...
    CSzDataw sd2 = *sd;                             // sauvegarder l'adresse où sera indiqué size
    Uint64toNumber(sd, temp64);
    tempPointer = sd->Data;                         // adresse de après size
    SZ_WRITE_BYTE(0)                                // external
    // liste des noms
